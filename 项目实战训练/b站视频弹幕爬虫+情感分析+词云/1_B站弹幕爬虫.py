...
def get_bilibili_danmu(bv, url, result_file):
	headers = {
	...
	df.to_csv(result_file, encoding='utf_8_sig', mode='a+', index=False, header=header)  # 数据保存到csv文件

def main():
	csv_file = '刘畊宏弹幕.csv'  				# 保存数据的文件名
	...
	# 开始爬取
	for bv in bv_list:
		get_bilibili_danmu(bv, url='https://www.bilibili.com/video/{}'.format(bv), result_file='刘畊宏弹幕.csv')
